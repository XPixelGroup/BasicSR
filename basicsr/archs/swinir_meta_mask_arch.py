...
class SwinIRMetaMaskUpsample(nn.Module):
    r""" SwinIRMeta
        A PyTorch impl of : `SwinIR: Image Restoration Using Swin Transformer`, based on Swin Transformer.
        Use Meta-SR as upsampler. Use masked data.
    ...
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(SwinIRMetaMaskUpsample, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
    ...
    def forward(self, x, lq_mask):
        ...
        if self.upsampler == 'meta':
            x = self.conv_first(x)
            # x = x * lq_mask
            # x = self.conv_after_body(self.forward_features(x) * lq_mask) + x
            x = self.conv_after_body(self.forward_features(x)) + x
            # x = x * lq_mask
