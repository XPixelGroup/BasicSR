...
def generate_mod_LR_bic():
    ...
    for i in range(num_files):
        ...
        if len(image.shape) == 3:
            ...
            image_HR = image[0:mod_scale*height, 0:mod_scale*width]
        # LR
        image_LR = cv2.resize(image_HR, None, fx=1/up_scale, fy=1/up_scale, interpolation = cv2.INTER_CUBIC)
        # bic
        image_Bic = cv2.resize(image_LR, None, fx=up_scale, fy=up_scale, interpolation = cv2.INTER_CUBIC)
